package net.corda.cordapp.java.jdk11.helloworld.contracts;
...
public class HelloWorldContract implements Contract {
    ...
    public void verify(LedgerTransaction tx) {
        ...
        final var commandData = tx.getCommands().get(0).getValue();

        if (commandData instanceof Commands.Send) {
            //Retrieve the output state of the transaction
            var output = tx.outputsOfType(HelloWorldState.class).get(0);
